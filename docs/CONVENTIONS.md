...
- All forms route to FormSubmit.co with target `gaurav@upcoretechnologies.com`, CC `saswata@upcoretechnologies.com`. The destination is hard-coded in **eight places** (see [ARCHITECTURE.md §7](ARCHITECTURE.md#7-external-services--destinations)): `assessment.html`, `contact.html`, `lp/maturity-review.html`, `lp/lead-magnet-engine.js`, `insights/index.html` (newsletter), `chat-widget.js` (`LEAD_EMAIL`/`LEAD_CC`), `api/build-demo.js` (`NOTIFY_TO`/`NOTIFY_CC`), and legacy/unused `api/chat.js` (`sendBookingEmails`). When changing it, change all of them (or at minimum the seven live ones).
- **FormSubmit activation gotcha — check this before assuming a new form is broken.** FormSubmit requires a one-time confirmation click the first time any new (recipient, subject) combination sends mail — this applies to the CC'd address too, independently of the primary recipient. A brand-new page's very first real submission is silently withheld (no error, no bounce) until someone clicks the "Activate your form" email FormSubmit sends. This bit `lp/maturity-review.html` on launch day (2026-08-06) — saswata@ didn't receive the first live submission despite the code being correct. **After deploying any new page with a FormSubmit form, do one real test submission and confirm both `gaurav@upcoretechnologies.com` and `saswata@upcoretechnologies.com` receive it (check spam too) before considering the form done.**
