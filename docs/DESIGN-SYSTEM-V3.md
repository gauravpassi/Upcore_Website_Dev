...
- **Nav**: `background:rgba(4,18,28,.72)` + `backdrop-filter:blur(14px)` (glassy, translucent, reveals a soft blur of whatever scrolls beneath it) + a cyan-tinted hairline bottom border (`rgba(33,210,237,.14)`, was plain white-opacity) + a soft drop shadow.
- **Footer**: `background` gained a `radial-gradient(620px 160px at 50% 0%, rgba(33,210,237,.09), transparent 70%)` layer sitting on top of `var(--ink)` — a soft cyan glow bleeding down from the top border, replacing the old flat 3px solid border (which was also still the pre-migration teal RGB, `rgba(10,191,204,...)` — now corrected to the V3 cyan `rgba(33,210,237,...)` at a hairline 1px weight, matching the "glow does the elevation, not a thick border" philosophy elsewhere in this doc).
- **Body**: `background:var(--bg)` became a 3-layer ambient "aurora" — three soft radial cyan glows (top-left, right-mid, bottom-center) over the flat navy, with `background-attachment:fixed` so the glows stay stationary while the page scrolls past them. This only shows through on sections that use `var(--bg)` directly (hero, CTA bands) — sections using `var(--bg2)`/`var(--bg3)`/`var(--card)` paint over it as before, so no section needed z-index changes; this was implemented as a pure `background` value, never a positioned pseudo-element, specifically to avoid any stacking-context risk across a 73-file bulk edit.

All three changes are pure CSS (no HTML/JS changes, no new stacking contexts), verified with fresh-tab screenshots + DOM checks (a stale-screenshot tool quirk from earlier in this session recurred here and was cross-checked against `getComputedStyle`/`getBoundingClientRect` before being ruled a non-issue).
