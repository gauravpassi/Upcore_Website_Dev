...
### A3d. AI Operations Toolkit (lead magnet — $3 lifetime)
- **What:** Landing page — "Stop Wasting Expensive Human Hours on Work AI Can Handle." Sells the **AI Operations Plug-and-Play Toolkit**: 10 specialized Kits combining 30+ Custom GPTs & Claude Skills built around real operational problems (waste-hunting, readiness scoring, workflow simplification, ROI calculation, workflow architecture, automate-or-human decisioning, 30-day planning). Sells for $3, lifetime access, per a teammate's real product at `ops-finder-pro.lovable.app`. Ends with a "Next step if you want it" cross-sell into the real FAO service (`/ai-adoption-strategy`), since the Toolkit only helps a buyer *identify* automation opportunities, not implement them.
- **Where:** [`ai-operations.html`](../ai-operations.html) (URL `/ai-operations`)
- **Design history:** Rebuilt 2026-08-21 from a bespoke standalone design the user supplied (own dark navy/cyan design system — `--bg:#071A26`, `--primary:#21D2ED`, Space Grotesk + DM Sans + JetBrains Mono, animated marquee ticker, JS-rendered kit cards, scroll-reveal). Per explicit instruction, the page's own `<style>` block, body content, and inline script were kept byte-for-byte as supplied — **only** the page's own `<header>`/`<footer>` were swapped for this site's real sitewide nav/footer (dark `#0a0a0a`, same markup as every other page). This reverted the previous build's "$3 → free" change; pricing is now $3 again, matching the supplied design exactly. The original file's two inline `data:image/png;base64` Upcore-logo blobs (identical, ~86KB each) were kept per explicit instruction — they're reused as-is in the new nav/footer `<img>` tags rather than swapped for `/upcore-logo.png`.
- **Touches:** Sitewide nav/footer CSS appended to the page's own `<style>` block (class names don't collide — the page uses `.foot-*`/`.svc-*` naming, the site uses `.footer-*`); GTM/GA4/Clarity tracking + favicons + Product/FAQPage JSON-LD added to `<head>` (JSON-LD price synced to 3 to match the visible price); chat-widget.js + cta-tracking.js added before `</body>`. Nav CTA points to `#get` (this page's own final pricing section), labeled "Get It for $3." No nav slot added (`fde-engineers.html` precedent).
- **Conversion mechanism (fixed 2026-08-21):** The `#get` section's own CTA used to be `<a href="#get">` — a dead anchor pointing at itself, with no way to actually capture a lead or a payment. Replaced with a real `<form id="getForm">` (name + email, FormSubmit.co, same pattern as `contact.html`) that notifies `gaurav@upcoretechnologies.com`/cc `saswata@upcoretechnologies.com` and confirms to the prospect that a secure $3 payment link is coming. Every other "Get it for $3" CTA on the page still anchors to `#get`, which now does something. **There is still no real Stripe/Gumroad checkout** — this collects the lead and promises a manual payment-link follow-up, not an automated charge. Also fixed: `href="#service"` on the "Talk to our engineers" link pointed at its own section; now goes to `/assessment`.
- **Liveliness/motion pass (2026-08-21):** Added — an idle "breathe" glow on `.cta` buttons (alpha-capped, pauses on hover, reuses the page's own negative-spread box-shadow convention rather than importing a different one); a slower ambient breathe on the two big price panels (`.get-card`, `.price-box`); hover-lift on all explorable content cards (kit cards, value cards, objection cards, process steps, question cards, toolbox rows/stats) — deliberately *not* applied to the "don't" list or static framing blocks; an arrow slide-on-hover on CTA icons; a sticky bottom conversion bar that appears once the visitor scrolls past the hero and hides again once they reach the real `#get` form (so it never stacks a duplicate CTA); and count-up animation on the six numeric stat/price call-outs (`10`, `30+`, three `$3`s, `10 Kits`), all gated behind `prefers-reduced-motion`.
- **Bug fixed 2026-08-21:** the supplied file's own inline script had `document.getElementById('hdr')` — a reference to the original `<header id="hdr">` that no longer exists after the nav/footer swap. It threw `Cannot read properties of null` on **every scroll event** site-visit-wide. Removed the dead header-scroll-state code (it only toggled a class the sitewide nav doesn't use anyway).
- **Extend by:** Inline edits within the page's own design system (don't reintroduce the sitewide `--teal`/DM-Sans-only token set — this page intentionally keeps its own `--primary:#21D2ED` cyan and Space Grotesk display face). If a real Stripe/Gumroad checkout gets built, replace `handleGetSubmit()`'s FormSubmit call rather than layering payment on top of it.
