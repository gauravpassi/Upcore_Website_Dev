...
- **Google Ads conversion event:** [`assessment.html`](../assessment.html) fires `gtag('event', 'conversion', {send_to: 'AW-16546427858/CN8BCPra5LsZENLn-dE9'})` — scoped to the existing `if(window.location.search.includes('submitted=true'))` block that already shows the success message post-submit (the FormSubmit `_next` redirect target), so it fires once per real completed Discovery Call booking, never on a normal page view. Verified in-browser: no conversion event on a plain visit, exactly one correct event on `?submitted=true`.
- **Where:**
