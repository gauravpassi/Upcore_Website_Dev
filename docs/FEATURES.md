...
### A3d. AI Operations Toolkit (free lead magnet)
- **What:** Free lead-magnet landing page — "Stop Wasting Expensive Human Hours on Work AI Can Handle." Sells (for free, lifetime access) the **AI Operations Plug-and-Play Toolkit**: 10 specialized Kits combining 30+ Custom GPTs & Claude Skills built around real operational problems (waste-hunting, readiness scoring, workflow simplification, ROI calculation, workflow architecture, automate-or-human decisioning, 30-day planning). Originally built and run for real by a teammate on Lovable (`ops-finder-pro.lovable.app`) as a $3 paid product — cloned content/structure 1:1 onto this site's design system per an explicit request, with the price changed from $3 to free. Ends with a "Next step if you want it" cross-sell into the real FAO service (`/ai-adoption-strategy`), since the Toolkit only helps a buyer *identify* automation opportunities, not implement them.
- **Where:** [`ai-operations.html`](../ai-operations.html) (URL `/ai-operations`)
- **Touches:** Standard nav/footer, chat widget, Product + FAQPage JSON-LD. No `#book-governance` — every CTA scrolls to an on-page `#get-access` email-capture form (FormSubmit.co → `gaurav@upcoretechnologies.com`, cc `saswata@upcoretechnologies.com`, same pattern as `contact.html`/`assessment.html`). No nav slot added (same precedent as `fde-engineers.html` — not every flagship-style page gets nav real estate); linked from the footer Solutions column on this page itself only so far.
- **Extend by:** Inline edits. **The Toolkit's actual delivery is manual** — the confirmation copy promises access "within 24 hours" and does not auto-send any files, since the 10 Kits/GPTs/Skills are the teammate's real assets, not something this repo generates or hosts. If that changes (e.g. an automated delivery mechanism is built), update both the form's success copy and this entry. Keep the "Free" framing consistent — do not reintroduce the original $3/$97 pricing anywhere on this page.
