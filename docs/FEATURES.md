...
- **⚠️ Known gap — no functional conversion mechanism.** As supplied, every "Get it for $3" / "Get my toolkit for $3" CTA on the page is `<a href="#get">` — an anchor link, including the one *inside* `#get` itself, which points at its own section. There is no Stripe/payment checkout, no email-capture form, nothing that actually collects a lead or a payment. This was kept as-is per explicit instruction, but the page currently cannot convert a visitor into anything. Needs a decision before real traffic: real $3 checkout (Stripe/Gumroad), or revert to the free email-gate pattern used in the prior build (FormSubmit.co, same as `contact.html`).
- **Extend by:** Inline edits within the page's own design system (don't reintroduce the sitewide `--teal`/DM-Sans-only token set — this page intentionally keeps its own `--primary:#21D2ED` cyan and Space Grotesk display face). Resolve the conversion-mechanism gap above before driving paid traffic here.
