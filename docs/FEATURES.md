...
- **Where:** [`lp/lead-magnet-engine.js`](../lp/lead-magnet-engine.js) — shared vanilla-JS IIFE (`window.LeadMagnetEngine.init(config)`) owning all mechanical logic: flow control, scoring, hand-rolled inline-SVG charts (no chart library), jsPDF generation (`_generatePdf`, with shared branding helpers `pdfHeaderBand`/`pdfFooter`/`pdfTierColor`), GTM events, UTM/gclid capture. [`lp/governance-index.html`](../lp/governance-index.html) and [`lp/ai-maturity-index.html`](../lp/ai-maturity-index.html) each own their own `<style>` block and an inline `NICHE_CONFIG` object (copy, question schema, tier table, chart config). `robots.txt` disallows `/lp/`.
- **Touches:** jsPDF (CDN `<script>`, `cdnjs.cloudflare.com/ajax/libs/jspdf`). [`api/lead-magnet-submit.js`](../api/lead-magnet-submit.js) is the real backend (both pages' `config.api.submit` points at `/api/lead-magnet-submit`): recomputes the score server-side (never trusts the client-submitted score/tier), POSTs one row per lead to a Google Sheet via a Google Apps Script Web App webhook (`GOOGLE_SHEETS_WEBHOOK_URL` — **not Apollo, not Vercel KV**; this account has no Vercel Pro plan, so the Sheet is also the peer-benchmark store — the Apps Script computes and returns the niche's running aggregate, count/avgOverall/avgByDim, in its own response), and separately notifies the team by email via FormSubmit.co (same service as every other lead form on this site — see §E, no new API key). If either the sheet write or the team email fails, the function still returns a valid score/tier response (`Promise.allSettled`, logged not thrown) — a lead never gets stuck because of a CRM outage. The client (`_submitContact` in the engine) itself also degrades gracefully: if the fetch to `/api/lead-magnet-submit` fails outright (network error, 404, etc.), it falls back to the locally-computed score with `peer.sufficientData:false` rather than blocking the visitor.
...
- **Analytics — GA4 + Google Ads:** `pushEvent()` in the engine fires every funnel event two ways: `window.dataLayer.push(...)` (for anyone who later wants GTM-based triggers) AND a direct `gtag('event', eventName, params)` call — since both pages already load `gtag.js` straight for GA4 (`G-TVRF5M70ES`), the direct call lands in GA4 Realtime immediately with zero GTM configuration required. Event names, per niche (`index_*` for governance, `maturity_*` for maturity — swap the prefix, params are identical):
  ...
  Every event also carries `niche` (`governance-index` / `ai-maturity-index`) for segmentation. **Google Ads conversion:** `config.googleAdsConversionLabel` (currently `null` on both pages, in the `NICHE_CONFIG` object) gates a `gtag('event','conversion',{send_to:...})` call fired at the same `assessment_complete` moment — same pattern as `assessment.html`'s and `chat-widget.js`'s existing conversions, same `AW-16546427858` account. **Manual step required:** create a "Assessment Complete" conversion action in Google Ads (Tools & Settings → Conversions → New), then fill the resulting `AW-16546427858/<label>` string into both pages' `googleAdsConversionLabel`. Until then the GA4 event still fires correctly — only the Ads-specific ping is skipped.
