...
- **Extend by:**
  ...
  - `config.screensPerView` controls two different UX patterns, not just a number of questions: `1` auto-advances the whole screen once every question on it is answered (used by the "console" skin's scan-log feel); `>1` renders an explicit disabled-until-complete `Continue` button (used by the "scorecard" skin's deliberate step-by-step feel). Both niches currently put 2 questions per screen either way — the difference is auto-advance vs. manual continue, not question count.
